...
import org.http4s.ember.testing.LockedTraits

private[core] object Http4sArbitraries extends LockedTraits {
  ...
  implicit val http4sTestingCogenForDecodeFailure: Cogen[DecodeFailure] =
  ...
  implicit def http4sTestingArbitraryForMessage[F[_]]: Arbitrary[Message[F]] =
    // TODO this is bad because the underlying generators are bad
    Arbitrary(Gen.oneOf(arbitrary[Request[F]], arbitrary[Response[F]]))

  implicit def http4sTestingArbitraryForRequest[F[_]]: Arbitrary[Request[F]] =
    Arbitrary {
  ...
  implicit def http4sTestingArbitraryForResponse[F[_]]: Arbitrary[Response[F]] =
    Arbitrary {
  ...
  private implicit class ParseResultSyntax[A](result: ParseResult[A]) {
    def yolo: A = result.fold[A](e => sys.error(e.toString), identity)
  }
