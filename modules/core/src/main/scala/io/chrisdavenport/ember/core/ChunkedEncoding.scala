...
object ChunkedEncoding {
  ...
  /** From fs2-http
    ...
    */
  def decode[F[_]: ApplicativeError[?[_], Throwable]](maxChunkHeaderSize:Int): Pipe[F, Byte, Byte] = {
  ...
  private val lastChunk: Chunk[Byte] = Chunk.ByteVectorChunk((ByteVector('0') ++ `\r\n` ++ `\r\n`).compact)
  ...
  def encode[F[_]]:Pipe[F,Byte,Byte] = {
    def encodeChunk(bv:ByteVector):Chunk[Byte] = {
      ...
      else Chunk.ByteVectorChunk(ByteVector.view(bv.size.toHexString.toUpperCase.getBytes) ++ `\r\n` ++ bv ++ `\r\n` )
    }
