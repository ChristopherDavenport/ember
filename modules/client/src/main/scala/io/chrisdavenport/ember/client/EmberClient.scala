...
final class EmberClient[F[_]: Bracket[?[_], Throwable]] private[client] (
  ...
  def expectOr[A](req: Request[F])(onError: Response[F] => F[Throwable])(
    ...
    fetch(r) {
      ...
      case failedResponse =>
        onError(failedResponse).flatMap(ApplicativeError[F, Throwable].raiseError)
    }
  }
    ...
    fetch(r) {
      ...
      case failedResponse =>
        failedResponse.status match {
          ...
          case _ => onError(failedResponse).flatMap(ApplicativeError[F, Throwable].raiseError)
        }
    }
  }
  ...
  def get[A](s: String)(f: Response[F] => F[A]): F[A] =
    Uri.fromString(s).fold(ApplicativeError[F, Throwable].raiseError, uri => get(uri)(f))
  ...
  def getAs[A](s: String)(implicit d: EntityDecoder[F, A]): F[A] =
    Uri.fromString(s).fold(ApplicativeError[F, Throwable].raiseError, uri => expect[A](uri))
